#!/usr/bin/env python3
"""Formal strict-split PlanB T=5 temporal baseline for B0 microscopy."""
...
DEFAULT_OUT_DIR = Path("/home/hjk4090d/microscopy_project/runs/b0_planb_t5_strict_baseline")
PATCH_SIZE = 512
WINDOW_SIZE = 5
OVERLAY_COUNT = 20
VALID_SPLITS = {"train", "val", "test"}
TIME_COLUMNS = ("t_minus2", "t_minus1", "t0", "t_plus1", "t_plus2")
...
class CsvColumns:
    ...
    frame_height: str
    frame_width: str
    center_foreground_ratio_patch: str
    center_has_foreground: str
...
class MetricAccumulator:
    ...
    def add(self, loss: float, pred: torch.Tensor, target: torch.Tensor) -> None:
        ...
        self.pred_sum += float(torch.sum(pred_bin).item())
        self.target_sum += float(torch.sum(target_bin).item())
    ...
    def metrics(self) -> dict[str, float | int]:
        ...
        return {
            "loss": self.loss_sum / self.num_windows,
...
def resolve_columns(fieldnames: list[str]) -> CsvColumns:
    values = {
        column: find_column(fieldnames, (column,), column)
...
def validate_row(row: dict[str, str], columns: CsvColumns) -> dict[str, Any]:
    ...
    center_t = parse_int(row[columns.center_t], columns.center_t, row_number)
    frame_indices = [
        parse_int(row[getattr(columns, column)], column, row_number)
        for column in TIME_COLUMNS
    ]
    foreground_ratio = parse_float(
        row[columns.center_foreground_ratio_patch],
        columns.center_foreground_ratio_patch,
        row_number,
    )
    ...
    if frame_indices[2] != center_t:
        raise ValueError(
            f"Row {row_number}: t0={frame_indices[2]} does not match center_t={center_t}"
        )
    ...
    return {
        ...
        "center_foreground_ratio_patch": foreground_ratio,
...
class TemporalWindowDataset(Dataset):
    ...
    def read_item_arrays(self, index: int) -> tuple[list[np.ndarray], np.ndarray, dict[str, Any]]:
        row = self.rows[index]
        values = validate_row(row, self.columns)
        row_number = int(values["row_number"])
        mcy_path = resolve_path(
            row[self.columns.mcy_path],
            self.csv_path,
            row_number,
            self.columns.mcy_path,
        )
        mask_path = resolve_path(
            row[self.columns.center_mask_path],
            self.csv_path,
            row_number,
            self.columns.center_mask_path,
        )

        image_patches: list[np.ndarray] = []
        for frame_index in values["frame_indices"]:
            image = read_tiff_page(mcy_path, int(frame_index))
            image_patch = crop_patch(
                ...
                PATCH_SIZE,
                PATCH_SIZE,
                f"image row {row_number} t={frame_index}",
            )
        ...
        meta = {
            ...
            "center_t": int(values["center_t"]),
            "frame_indices": [int(item) for item in values["frame_indices"]],
            "center_foreground_ratio_patch": float(values["center_foreground_ratio_patch"]),
            "density_bucket": str(values["density_bucket"]),
            "mcy_path": str(mcy_path),
            "center_mask_path": str(mask_path),
        }
        if not meta["sample_id"]:
            raise ValueError(f"Row {row_number}: empty sample_id")
        return image_patches, mask_patch, meta

    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
        image_patches, mask_patch, meta = self.read_item_arrays(index)
        image_stack = np.stack(
            [percentile_normalize(patch)[None, :, :] for patch in image_patches],
            axis=0,
        )
        image_tensor = torch.from_numpy(np.ascontiguousarray(image_stack)).float()
...
class PlanBT5UNet(nn.Module):
    def __init__(self, in_channels: int = WINDOW_SIZE, out_channels: int = 1, base_channels: int = 16) -> None:
        super().__init__()
        c = base_channels
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 5:
            raise ValueError(f"PlanBT5UNet expects [B, 5, 1, H, W], got {tuple(x.shape)}")
        if x.shape[1] != WINDOW_SIZE or x.shape[2] != 1:
            raise ValueError(f"PlanBT5UNet expects [B, 5, 1, H, W], got {tuple(x.shape)}")
        x = x[:, :, 0, :, :]
        e1 = self.enc1(x)
...
def split_rows_by_csv(
    ...
    for split, cap in caps.items():
        if cap is not None:
            if cap <= 0:
                raise ValueError(f"--max-{split}-windows must be positive when provided")
...
def train_one_epoch(
    ...
    return loss_sum / window_count, global_step
...
def generate_prediction_overlays(
    model: nn.Module,
    datasets: list[tuple[str, TemporalWindowDataset]],
    out_dir: Path,
    device: torch.device,
    use_amp: bool,
    ...
    with torch.no_grad():
        for split_name, dataset in datasets:
            ...
            for index in range(min(len(dataset), per_split_limit)):
                if len(saved) >= total_count:
                    break
                image_patches, mask_patch, meta = dataset.read_item_arrays(index)
                image_stack = np.stack(
                    [percentile_normalize(patch)[None, :, :] for patch in image_patches],
                    axis=0,
                )
                image_tensor = torch.from_numpy(np.ascontiguousarray(image_stack[None, :, :, :, :])).float()
                with autocast(enabled=use_amp):
                    logits = model(image_tensor.to(device))
                pred = (torch.sigmoid(logits)[0, 0].detach().cpu().numpy() >= 0.5).astype(np.uint8)
                gt = (mask_patch > 0).astype(np.uint8)

                center_image = image_stack[2, 0]
                base = (np.clip(center_image, 0.0, 1.0) * 255).astype(np.uint8)
                ...
                filename = f"{split_name}_{len(saved):03d}_{sample}_t{meta['center_t']}.png"
...
def build_datasets(args: argparse.Namespace) -> tuple[dict[str, TemporalWindowDataset], CsvColumns, Path]:
    ...
    datasets = {
        split: TemporalWindowDataset(split_rows[split], columns, csv_path, split)
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(out_dir / "planb_t5_strict_config.json", config)

    best_path = out_dir / "planb_t5_strict_best.pt"
    last_path = out_dir / "planb_t5_strict_last.pt"
    best_val_dice = -1.0
    best_epoch = 0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        epoch_row = {
            ...
            "val_num_windows": val_metrics["num_windows"],
        }
    ...
    write_rows_csv(
        out_dir / "planb_t5_strict_epoch_log.csv",
        epoch_rows,
        [
            "epoch",
            "train_loss",
    ...
    write_rows_csv(
        out_dir / "planb_t5_strict_split_metrics.csv",
        split_metric_rows,
        ["split", *metric_fields],
    )
    write_rows_csv(
        out_dir / "planb_t5_strict_sample_metrics.csv",
        sample_metric_rows,
        ["split", "sample_id", *metric_fields],
    )
    write_rows_csv(
        out_dir / "planb_t5_strict_density_bucket_metrics.csv",
        bucket_metric_rows,
    ...
    summary = {
        ...
        "outputs": {
            ...
            "epoch_log": out_dir / "planb_t5_strict_epoch_log.csv",
            "split_metrics": out_dir / "planb_t5_strict_split_metrics.csv",
            "sample_metrics": out_dir / "planb_t5_strict_sample_metrics.csv",
            "density_bucket_metrics": out_dir / "planb_t5_strict_density_bucket_metrics.csv",
            "config": out_dir / "planb_t5_strict_config.json",
            "prediction_overlays": overlays,
        },
        "config": config,
    }
    write_json(out_dir / "planb_t5_strict_summary.json", summary)
    return summary
...
def main() -> int:
    args = parse_args()
    run_training(args)
    print("PLANB_T5_STRICT_BASELINE_OK")
    return 0
